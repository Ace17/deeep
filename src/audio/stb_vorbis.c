...
static_assert(STB_VORBIS_MAX_CHANNELS <= 256, "Value of STB_VORBIS_MAX_CHANNELS outside of allowed range");
static_assert(STB_VORBIS_FAST_HUFFMAN_LENGTH <= 24, "Value of STB_VORBIS_FAST_HUFFMAN_LENGTH outside of allowed range");
...
static int8 integer_divide_table[DIVTAB_NUMER][DIVTAB_DENOM]; // 2KB
#endif
...
// WINDOWING
  ...
  const int n = f->blocksize[m->blockflag];
  Mapping* map = &f->mapping[m->mapping];
...
{
  return (stb_vorbis*)setup_malloc(f, sizeof(stb_vorbis));
}
...
int stb_vorbis_get_frame_float(stb_vorbis* f, int* channels)
{
  int len, right, left;
...
int stb_vorbis_get_samples_float_interleaved(stb_vorbis* f, int channels, float* buffer, int num_floats)
{
  ...
  while(n < len)
  {
    ...
    if(!stb_vorbis_get_frame_float(f, NULL))
      break;
  }
